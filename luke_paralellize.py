...
def generate_single_sample(args):
    """
    Generate data samples for a single instance.

    Args:
        args (tuple): A tuple containing (word_list, max_word_length).
...
def generate_training_data(word_list, num_samples, max_word_length=10):
    """
    Generate synthetic training data for the Hangman game in parallel.
...
def train_model(word_list, model_path="hangman_model_cpu_parallel.pth", num_samples=20000, epochs=25, batch_size=32, lr=0.001):
    device = torch.device("cpu")  # Force CPU usage

    # Optionally set number of threads for CPU computations
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=4)  # num_workers=4 for data loading parallelism
    ...
    print(f"Total training time (CPU-parallelized): {total_training_time:.2f} seconds")
