...
class Train_Model():
    ...
    def __init__(self, model_path, num_workers) -> None:
        ...
        self.train_model(word_list, model_path=model_path, num_workers=num_workers)
        print("Training completed.")

    def generate_sample(self, _, word_list, max_word_length):
        ...
        while len(guessed_letters) < len(word_letters):
            ...
            word_input = self.encode_word_state(''.join(obscured_word), max_word_length)
            guessed_input = self.encode_guessed_letters(guessed_letters)
    ...
    def generate_training_data(self, word_list, num_samples, max_word_length=10):
        """Generate training data based on word list in parallel."""
        print("Generating training data...")

        # Parallize
        with multiprocessing.Pool() as pool:
            data = pool.starmap(self.generate_sample, [(i, word_list, max_word_length) for i in range(num_samples)])
    ...
    def encode_word_state(self, word_display, max_word_length):
    ...
    def train_model(self, word_list, model_path, num_workers):
        ...
        data = self.generate_training_data(word_list, num_samples)
